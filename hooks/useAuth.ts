...
import { enforcePlatinumExpiry, enforceVerificationExpiry } from "@/lib/verification";
  ...
  onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
      unsubscribeProfile = subscribeToUserProfile(user.uid, (profile) => {
        ...
        // Hourly Platinum ends on its own; see enforcePlatinumExpiry.
        if (profile) void enforcePlatinumExpiry(user.uid, profile);
        setLoading(false);
      });
      setOnline(user.uid);
